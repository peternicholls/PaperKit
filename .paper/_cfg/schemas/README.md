...
```json
{
  ...
  "path": ".paper/module/agents/name.md"  // Required, validated pattern
}
```
...
- `path`: Must match `^\.paper/(core|specialist)/agents/[a-z][a-z0-9-]*\.md$`
...
# Load schema
with open('.paper/_cfg/schemas/agent-schema.json') as f:
    schema = json.load(f)
...
const schema = JSON.parse(
  fs.readFileSync('.paper/_cfg/schemas/agent-schema.json', 'utf8')
);
