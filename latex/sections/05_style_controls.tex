...
Temperature biases the journey's hue path toward warm or cool colors~\cite{hunt2004}. This control operates in the hue dimension of OKLCh (\S\ref{sec:oklch}), biasing the path selection when multiple hue routes are possible.
...
Temperature is implemented via direction blending. The direction vectors are normalized to ensure consistent blending behavior regardless of anchor placement:
\begin{equation}
    \vec{v} = (1 - \alpha|T|) \cdot \hat{v}_{\text{base}} + \alpha|T| \cdot \hat{v}_{\text{warm/cool}}
\end{equation}

where $\hat{v}_{\text{base}}$ is the \emph{normalized} shortest-path direction, $\hat{v}_{\text{warm/cool}}$ is the \emph{normalized} target hue direction, and $\alpha \in (0, 1)$ is a blending coefficient (typically $\alpha = 0.8$) that ensures partial preservation of the base direction, preventing jarring snaps to pure warm or cool paths. The normalization is essential: without it, the velocity of the resulting curve would vary based on anchor separation, potentially disrupting arc-length estimation performance.
...
\begin{itemize}
    ...
    \item \textbf{High intensity} ($0.7$--$1.0$): Dramatic arcs; pronounced curves through color space
\end{itemize}
...
Intensity affects path geometry, not endpoint colors. The anchors remain unchanged (\S\ref{sec:anchors}); only the route between them becomes more or less dramatic. This is achieved by scaling the Bézier control point offset (\S\ref{sec:bezier}).
...
Range $-1.0$ to $+1.0$, default $0$. Shifts all colors lighter (positive) or darker (negative). Applied proportionally to avoid clipping:
\begin{equation}
    L' = \begin{cases}
...
\paragraph{Chroma.}
Range $0.0$ to $2.0$, default $1.0$. Multiplies the chroma (saturation) of all colors. Values $>1$ increase saturation; values $<1$ desaturate toward grayscale.

\paragraph{Contrast.}
Range $0.0$ to $2.0$, default $1.0$. Expands or compresses the lightness range. High contrast darkens darks and lightens lights; low contrast pushes toward mid-gray. Implemented via S-curve:
\begin{equation}
...
\paragraph{Vibrancy.}
Range $0.0$ to $2.0$, default $1.0$. Selectively boosts chroma of less-saturated colors while leaving already-vibrant colors unchanged~\cite{poynton2012}. Useful for counteracting desaturation in complementary-color interpolation.
\begin{equation}
    C' = C \cdot \left(1 + v \cdot \left(1 - \frac{C}{C_{\max}}\right)\right)
\end{equation}

where $C_{\max}$ is the maximum achievable chroma at the color's current lightness $L$ and hue $h$ within the sRGB gamut boundary (see \S\ref{sec:gamut-problem} for gamut constraints).
