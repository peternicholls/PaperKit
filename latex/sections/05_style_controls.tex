...
Temperature biases the journey's hue path toward warm or cool colours.
...
Intensity affects path geometry, not endpoint colours. The anchors remain unchanged; only the route between them becomes more or less dramatic.
...
\paragraph{Vibrancy.}
Range $0.0$ to $2.0$, default $1.0$. Selectively boosts chroma of less-saturated colours while leaving already-vibrant colours unchanged. Useful for counteracting desaturation in complementary-colour interpolation.
\begin{equation}
...
The engine applies all dynamics as specified, then performs gamut mapping. Conflicts are resolved by clamping to valid values, not by preventing the combination. Diagnostics report when settings caused gamut corrections.
