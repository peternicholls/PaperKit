...
The \textbf{Just Noticeable Difference (JND)} is the smallest colour change that an average human observer can detect under standard viewing conditions. This concept originates from Weber's and Fechner's foundational psychophysics work and has been extensively studied in colour science \citep{fairchild2013}.
...
Notably, the CSS Color Level 4 gamut mapping algorithm uses a similar threshold concept: colours are considered ``close enough'' to clip when $\Delta E < 0.02$ on OKLab's 0--1 scale \citep{csscolor4}, which corresponds to approximately 2 units on the 0--100 scale commonly used in colour difference literature. Our threshold choice aligns with this industry practice.
