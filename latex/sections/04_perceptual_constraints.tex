...
The \textbf{Just Noticeable Difference (JND)} is the smallest color change that an average human observer can detect under standard viewing conditions. This concept originates from Weber's and Fechner's foundational psychophysics work and has been extensively studied in color science~\cite{fairchild2013}.
...
\textbf{Why 5.0?} This value represents the upper bound of ``comfortable'' color transitions---steps large enough to be clearly distinct but not so large as to feel discontinuous. The 5.0 threshold emerges from the JND interpretation table (\S\ref{sec:jnd}): differences above $\sim$5 units cross from ``obvious but smooth'' into ``pronounced step'' territory. This is a \textbf{proposed engineering constant} informed by practical experimentation with generated palettes during reference implementation testing, not a value derived from formal perceptual studies. Alternative implementations may choose different thresholds based on their aesthetic goals; formal user studies to validate this threshold remain future work.
