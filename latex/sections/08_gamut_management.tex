...
OKLab is a perceptually uniform color space, but it encompasses colors that lie outside any real display's capabilities \citep{morovic2008}:

\begin{itemize}
    \item \textbf{High-chroma colors} at certain hue angles exceed sRGB
    \item \textbf{Very dark or very light saturated colors} clip
...
The second layer handles residual out-of-gamut colors through \textbf{soft correction} at sampling time. Our approach aligns with the CSS Color Level 4 gamut mapping algorithm \citep{csscolor4}, which has been implemented in reference libraries such as Color.js \citep{colorjs2025}.
...
\textbf{Rationale:} Post-correction alone can introduce perceptual discontinuities when many samples need large corrections. Prevention minimises corrections needed; correction handles residual cases gracefully. This approach follows established best practices in color management \citep{morovic2008}.
...
\begin{itemize}
    \item Number of colors requiring correction
