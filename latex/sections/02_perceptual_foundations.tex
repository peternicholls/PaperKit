...
Human vision perceives colour differences nonlinearly. A linear blend in RGB space does not produce a linear perceptual transition---interpolating between saturated blue and yellow often produces washed-out greys or unexpected hues at the midpoint, because the intermediate colours pass through an unsaturated region of colour space.
...
To address this fundamental mismatch, scientists have developed \emph{perceptually uniform} colour spaces where Euclidean distances correspond more closely to perceived colour differences \cite{cie1976,fairchild2013}. In such spaces, moving a given distance in any direction produces a change that observers judge to be of similar magnitude.

The engine \emph{must} work in a perceptually uniform space so that equal steps along the journey correspond to equal perceived differences. This is non-negotiable for achieving smooth, natural colour transitions.

% ------------------------------------------------------------------------------
% 2.2 OKLab Colour Space
% ------------------------------------------------------------------------------
\section{OKLab Colour Space}
\label{sec:oklab}

OKLab, introduced by Björn Ottosson in 2020 \cite{ottosson2020}, is a modern perceptual colour space designed to predict human colour perception accurately while remaining computationally efficient. It has gained rapid industry adoption, appearing in CSS Color Level 4 \cite{csscolor4} and various creative tools. Independent analysis has validated its perceptual uniformity claims, particularly for gradient interpolation and colour manipulation tasks \citep{levien2021oklab}.

Compared to alternatives \citep{mahy1994,safdar2017}, OKLab offers an excellent balance. The following comparison synthesises Ottosson's original analysis \citep{ottosson2020} with established colour science literature \citep{fairchild2013}:

\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Colour Space} & \textbf{Uniformity} & \textbf{Cost} & \textbf{Stability} & \textbf{Selected} \\
\midrule
...
\textit{Uniformity} refers to how closely Euclidean distances match perceived differences; \textit{Cost} measures computational complexity (matrix operations vs.\ iterative solving); \textit{Stability} indicates behaviour at gamut boundaries and extreme lightness values. OKLab achieves CAM16-level uniformity \citep{safdar2017} with CIELAB-level computational cost---a key factor for real-time applications.

A colour in OKLab is represented by three coordinates $(L, a, b)$:
...
The perceptual distance between two colours is computed as Euclidean distance:
...
Ottosson designed OKLab such that a $\Delta E$ of approximately 1.0 corresponds to a just-noticeable difference (JND) for an average observer under standard viewing conditions \citep{ottosson2020}. This design goal aligns with the broader colour science literature on JND thresholds \citep{luo2001,fairchild2013}, though direct perceptual validation studies specific to OKLab remain limited. The 1.0 threshold is a design target rather than an empirically-validated constant across all viewing conditions.
...
For certain operations---particularly hue manipulation---a cylindrical form is more convenient. OKLCh represents the same colours using:

\begin{description}
    \item[$L$ --- Lightness] identical to OKLab
    \item[$C$ --- Chroma] defined as $C = \sqrt{a^2 + b^2}$, representing distance from the neutral axis (colourfulness/saturation)
...
\begin{itemize}
    ...
    \item \textbf{Cylindrical (OKLCh)} for hue-related manipulations such as warmth bias and complementary colour calculations
\end{itemize}
...
% ------------------------------------------------------------------------------
\section{Colour Space Conversion}
...
Each step models aspects of human vision: $M_1$ approximates L, M, S cone responses; the cube root models nonlinear perceptual compression; $M_2$ extracts lightness as a weighted average and opponent-colour signals as cone differences.
...
\begin{enumerate}
    \item \textbf{Linear interpolation works} --- A straight line between two colours in OKLab produces a perceptually smooth gradient
...
Because OKLab is unbounded---it can represent colours outside any physical display gamut---the engine must include explicit gamut mapping (\S\ref{sec:gamut-problem}).
