...
Every domain working with color—UI design, data visualisation, generative art—requires coherent palettes. Yet producing them is deceptively hard. Ad-hoc methods (RGB interpolation, color wheels) fail fundamentally: adjacent colors feel inconsistent, some nearly identical while others jump dramatically. This is not a cosmetic problem, it breaks user experience and data clarity.
...
\begin{itemize}
    \item \textbf{\S\ref{sec:perceptual-foundations}:} Perceptual foundations—Riemannian structure, OKLab space, and temporal perception
    \item \textbf{\S\ref{sec:journey-metaphor}:} Journey construction---anchors, Bézier curves, and sampling
    \item \textbf{\S\ref{sec:jnd}:} Perceptual constraint framework
    ...
    \item \textbf{\S\ref{sec:summary}:} Summary and future directions
\end{itemize}
