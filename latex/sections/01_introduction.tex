...
Rather than failing on unusual inputs, the engine adapts. Gray anchors expand along the lightness axis; extreme colors adjust their expansion direction; pathological inputs produce documented fallback behavior.
...
\subsection*{Prior Art: Color Harmony and Palette Generation}

Traditional palette generation approaches fall into two categories. First, \emph{color harmony rules}---complementary, triadic, analogous, split-complementary---define geometric relationships on the color wheel~\cite{itten1961}. These rules have aesthetic merit and deep historical roots in art education, but they operate in perceptually non-uniform spaces (typically HSL or HSV) and provide no guarantees about perceived differences between generated colors.

Second, \emph{interpolation-based approaches} generate colors by blending between endpoints. Most creative tools (CSS gradients, design software) use linear interpolation in RGB or HSL space, producing results that can appear perceptually uneven---muddy midpoints, abrupt hue shifts through gray regions, and inconsistent step sizes~\cite{stone2014}.

\subsection*{The Shift to Perceptual Color Spaces}

Modern tools increasingly adopt perceptually uniform color spaces to address these limitations. CIELAB~\cite{cie1976} was the first widely-used perceptual space but exhibits known uniformity issues, particularly in blue-violet regions~\cite{mahy1994}. CIEDE2000~\cite{luo2001} improved color difference calculation but remains computationally complex.
...
\begin{itemize}
    ...
    \item \textbf{Single-anchor mood expansion}---to our knowledge, a novel method---using lightness-weighted directionality in perceptual space to generate coherent palettes from one color, rather than relying on geometric harmony rules
    \item \textbf{Two-layer gamut management}~\cite{morovic2008} preventing and correcting out-of-gamut colors while preserving hue
\end{itemize}

Our contribution lies not in the individual techniques---OKLab, Bézier curves, arc-length sampling, and gamut mapping are established---but in their integration into a coherent system with explicit perceptual constraints and a deterministic, reproducible API. The resulting implementation achieves 5.6 million colors per second, enabling real-time palette generation.
