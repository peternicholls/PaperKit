...
Categorical Mode optimizes for \textbf{maximum distinguishability} between swatches, suitable for discrete data categories, legends, and accessibility-critical applications \citep{fairchild2013}. Unlike Journey Mode, which optimizes adjacent-color relationships (\S\ref{sec:delta-min}), Categorical Mode optimizes all pairwise relationships.
...
Fast hue swings feel ``faster'' or more noticeable than the same $\Delta E$ spent on subtle lightness shifts. The \emph{type} of change affects perceived speed, not just the magnitude. This observation is consistent with color appearance research showing that hue changes are more salient than equivalent lightness or chroma changes \citep{fairchild2013}.
