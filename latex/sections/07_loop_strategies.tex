...
% ------------------------------------------------------------------------------
The previous sections established \emph{what} the engine optimizes for---smooth sequential flow in Journey Mode or maximum categorical distinction---and \emph{how} style controls shape the journey's character. This chapter addresses a complementary concern: \emph{what happens at the boundaries} when a palette must extend beyond its natural start-to-end path, whether through cycling, reversal, or progressive evolution.
...
In closed loop mode, the output array \emph{omits the duplicate final color}. The last swatch is adjacent to (but not identical to) $A_1$, because $A_1$ is already the first swatch. When looped, the sequence wraps seamlessly.
...
Ping-pong is appropriate for ``breathing'' or ``pulsing'' color effects, bidirectional progress indicators, and oscillating state visualizations.
...
The \textbf{Möbius} strategy is a half-twist loop requiring \emph{two complete traversals} to return to the starting color---named after the Möbius strip, a surface with a half-twist where walking one complete loop brings you to the ``other side,'' requiring a second loop to return to your starting position.

In color terms: after one cycle through the journey, you arrive at a color \emph{related to} but distinct from the start; only after the second cycle do you return to the original. This creates animations that feel subtly different on alternating cycles.

The chromatic twist is implemented by \emph{gradually} inverting the chromatic components over the course of each loop---not as an instantaneous jump. The transformation progresses smoothly along the path:
\begin{equation}
    J_{\text{mobius}}(t) = (L(t), \, (1-2t) \cdot a(t), \, (1-2t) \cdot b(t)) \quad \text{for } t \in [0, 1]
\end{equation}

At $t=0$, the color is $(L, a, b)$; at $t=1$, it smoothly arrives at $(L, -a, -b)$---the complementary color (180° hue rotation). The $\Delta_{\max}$ constraint (\S\ref{sec:delta-max}) is enforced throughout this gradual transformation, ensuring that adjacent swatches remain within the coherence threshold even across the conceptual ``wrap point.'' The viewer perceives continuous change that takes two ``laps'' to truly repeat:
\begin{itemize}
...
Phased loops are parameterised by shift amount and dimension. This is the most complex loop type, appropriate only when continuous evolution is specifically desired---slowly evolving animations, generative art with progressive change, and long-running visualizations that should not feel static.
...
\textbf{Choice:} All loop strategies output a flat array of $N$ colors. Loops are ``unrolled'' into a sequential list with no nested structure or cycle metadata.
...
Loop outputs omit duplicate colors at boundaries. If the loop returns to the starting color, that color is \emph{not} repeated at the end of the array:
