...
The engine is a \textbf{pure function} that transforms configuration into palette \citep{bloch2008}:
...
The guiding principle: the engine should do only what the caller cannot do themselves. Complexity belongs in construction, not in the public interface \citep{gamma1994}.
...
\midrule
\texttt{count} & integer & Yes & Number of colors to generate \\
\texttt{anchors} & array & Yes & 1--5 colors (hex, RGB, or OKLab) \\
...
\texttt{gamutCorrections} & Count of colors requiring gamut mapping \\
...
Throughput & 5.6 million colors/second\footnote{Measured on Apple M1 hardware, single-threaded, using default configuration (2 anchors, 8 colors, journey mode). Performance varies with configuration complexity and hardware.} \\
...
\begin{itemize}
    ...
    \item Sampling: $O(n)$ for $n$ colors with constraint checking
\end{itemize}
