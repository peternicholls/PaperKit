...
\begin{itemize}
    ...
    \item \textbf{Focused} --- Does one thing well (colour journey generation)
...
\paragraph{Palette Array.} An ordered list of colour swatches, each containing:
\begin{itemize}
...
\textbf{Choice:} The engine core owns palette generation only. Colour naming, accessibility checking (WCAG), colour blindness simulation, and ICC colour management are explicitly caller responsibilities.
...
\begin{itemize}
  ...
  \item \textit{Built-in colour naming} --- Rejected: naming systems are culturally/contextually specific
  \item \textit{Colour blindness simulation} --- Rejected: simulation algorithms evolve independently
\end{itemize}
...
\midrule
Per-colour generation & Microsecond range \\
