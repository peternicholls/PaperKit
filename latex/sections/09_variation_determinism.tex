...
Anchors are \emph{never} perturbed---variation applies only to interpolated colors. This ensures user-specified key colors remain exact (\S\ref{sec:anchors}). Perturbations taper near anchors (scaling by $\sin(\pi t)$) for smooth transitions.
...
The variation layer uses a \textbf{pseudo-random number generator (PRNG)} with these properties \citep{knuth1997}:
...
\begin{itemize}
    ...
    \item \textbf{Specified algorithm} --- Implementation uses documented algorithm (e.g., xoshiro256** \citep{blackman2018})
\end{itemize}

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Seed Value} & \textbf{Behavior} \\
\midrule
