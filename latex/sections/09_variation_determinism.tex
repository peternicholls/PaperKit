...
The Color Journey Engine \emph{must} be programmatically deterministic. Given the same inputs, it \emph{must} produce the same outputs, up to small numerical differences ($\sim$0.2\%) arising from floating-point arithmetic and platform differences.
...
Anchors are \emph{never} perturbed---variation applies only to interpolated colours. This ensures user-specified key colours remain exact. Perturbations taper near anchors (scaling by $\sin(\pi t)$) for smooth transitions.
...
The variation layer uses a \textbf{pseudo-random number generator (PRNG)} with these properties:
...
\begin{itemize}
    ...
    \item \textbf{Specified algorithm} --- Implementation uses documented algorithm (e.g., xoshiro256**)
\end{itemize}
...
\texttt{null} or omitted & Default seed (0) or variation disabled \\
\bottomrule
\end{tabular}
\end{center}

If variation is requested but no seed provided, the engine uses a \emph{default seed (0)}, not a random seed. This preserves determinism. If you want randomness, you must generate and provide the seed.

The order in which random numbers are consumed (per-colour L, a, b perturbations) is fixed and documented---changing advancement order would change outputs for existing seeds.
