...
The most basic usage: interpolate between two colours to create a smooth gradient.
...
// Result: 5 colours from coral to teal
// [
...
// Intermediate colours flow smoothly between them
\end{lstlisting}

\section{Single-Anchor Expansion}
\label{sec:example-single}

Single-anchor mode (\S\ref{sec:single-anchor}) generates harmonious variations from one colour, useful for UI state variations and monochromatic palettes.
...
\subsection{Brand Colour State Variants}

Generate hover, active, and disabled states from a single brand colour.
...
Categorical Mode (\S\ref{sec:categorical-mode}) maximises perceptual distance between all colours, essential for data visualisation.
...
// Result: 6 maximally distinct colours
...
Pin specific semantic colours while filling remaining slots.
...
// Two additional colours generated with maximum distinction
\end{lstlisting}
...
// Last colour transitions smoothly to first
// Output: 12 colours, no duplicate at end
...
// 16 colours cover full forward-backward cycle
...
console.log(result.palette);      // Array of 8 colour objects
...
\subsection{Accessing Colour Formats}

\begin{lstlisting}[language=JavaScript,caption={Using Output Colour Formats}]
const result = generatePalette({
  count: 5,
  anchors: ['#FF6B6B', '#4ECDC4']
});
...
if (diag.gamutCorrections > 0) {
  console.log('Some colours were desaturated to fit sRGB gamut');
}
