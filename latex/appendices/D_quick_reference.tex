...
Performance & \S\ref{sec:performance} & Microsecond-range; 5.6M colors/second \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{API scope} (\S\ref{sec:api-scope}): Engine owns core generation; naming, accessibility, and color blindness simulation are caller responsibilities.
\end{itemize}
...
\midrule
\texttt{count} & $\mathbb{Z}^+$ & (required) & Number of output colors \\
\texttt{anchors} & 1--5 colors & (required) & Key colors to pass through \\
...
\texttt{diagnostics.gamutCorrections} & Count of gamut-mapped colors \\
...
Chart legend colors & categorical & \texttt{categorical} \\
...
Soft, approachable colors & journey & \texttt{pastel} \\
\bottomrule
\end{tabular}
...
\midrule
Colors look washed out & High chroma caused gamut mapping; reduce \texttt{chroma} \\
Unexpected color in middle & Temperature bias routing through different hues; check \texttt{temperature} \\
Visible ``jump'' between colors & Step size exceeds $\Delta_{\max}$; add anchors or increase \texttt{count} \\
