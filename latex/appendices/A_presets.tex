...
\paragraph{\texttt{smooth}} Gentle, flowing gradients with high smoothness for seamless $C^1$ continuity at anchor junctions. Appropriate for backgrounds, ambient effects, and contexts where the colour transition should feel natural and unobtrusive.
...
\paragraph{\texttt{muted}} Subdued palettes with low intensity and high smoothness. Produces sophisticated, understated colour combinations suitable for professional interfaces, document themes, and contexts requiring visual restraint.
...
\paragraph{\texttt{forest}} Moderate cool bias with natural green emphasis. Lower intensity preserves the organic, muted quality of woodland colours without artificial saturation.

\paragraph{\texttt{autumn}} Warm bias with higher intensity to capture the vibrant oranges, reds, and golds of fall foliage. Slightly lower smoothness allows for more distinct colour ``stops'' resembling individual leaves.
...
\paragraph{\texttt{pastel}} Low intensity with very high smoothness produces soft, desaturated palettes. The colours feel gentle and approachable---suitable for children's applications, wellness themes, or light UI backgrounds.

\paragraph{\texttt{earth}} Warm bias with restrained intensity for natural, organic colour combinations. Browns, tans, and warm neutrals dominate, suitable for outdoor, craft, or sustainability themes.

\paragraph{\texttt{monochrome}} Minimal intensity (0.1) with near-maximum smoothness for single-hue depth exploration. When combined with a single anchor (\S\ref{sec:single-anchor}), produces sophisticated tonal variations of one colour.

\subsection{Data Visualisation Presets}

\paragraph{\texttt{categorical}} Uses Categorical Mode (\S\ref{sec:categorical-mode}) to maximise perceptual distance between all colour pairs. Essential for charts, legends, and any context where colours represent distinct categories that must be easily distinguished.
...
\paragraph{\texttt{sequential}} Optimised for ordered data (low-to-high, early-to-late). Smooth progression with moderate intensity creates clear directionality in the palette.

\subsection{Animation Presets}

\paragraph{\texttt{breathing}} Ping-pong loop (\S\ref{sec:loop-pingpong}) with very high smoothness for pulse effects. The colour smoothly advances then retreats, creating a ``breathing'' rhythm without any discontinuity at reversal points.
...
Tonal variations of one colour & \texttt{monochrome} \\
\bottomrule
\end{tabular}
\end{center}
